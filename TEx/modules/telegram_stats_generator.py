...
class TelegramStatsGenerator(BaseModule):
    ...
    async def run(self, config: ConfigParser, args: Dict, data: Dict) -> None:
        ...
        stats_total_groups: int = 0 + len(groups)
        ...
        stats_total_active_users = TelegramMessageDatabaseManager.count_active_users(
    ...
    async def __render_media_stats(self, file: TextIOWrapper, max_large_group_name: int,
                                   stats_messages_per_groups: List[Dict]) -> None:
    ...
    async def __render_messages_stats(self, file: TextIOWrapper, max_large_group_name: int,
                                      stats_messages_per_groups: List[Dict]) -> None:
    ...
    async def __render(self, params: RenderParams) -> None:
        ...
        stats_avg_messages_day = params['stats_total_messages'] / datetime.timedelta(
            seconds=params['limit_seconds']).days

        # Define the Largest Group Name
        max_large_group_name: int = (
            max(len(item['group']) for item in params['stats_messages_per_groups'])
            + 1
        )
        ...
        with open(f'{params["report_root_folder"]}/stats.txt', 'wt', encoding='utf-8') as file:
            ...
            start = (datetime.datetime.now(tz=pytz.UTC) - datetime.timedelta(seconds=params['limit_seconds'])).strftime(
                '%Y-%m-%d %H:%M:%S')
